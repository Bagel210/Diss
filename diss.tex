...
The goal of this project was to build a system that is capable of synthesising life-like, natural head motions from text and that is what this project has achieved. The results from the evaluation showed that the random system was deemed the most natural of head motions, even over the real life recordings. This may be due to the fact that the environment in which the data recordings took place was a not a typical environment where head motions would be observed like in a real life conversation. Due to this the real life recordings scored worse than expected. 
...
Although the rule-based system showed promising initial results in prototypes it scored very poorly in the evaluation due to discontinuous, sharp nature of it's head motions. This c


\subsection{Data Driven System}

Due to time constraints the project never reached a data driven system. As outlined previously in the related works section many existing system's are using Hidden Markov Models as a tool to develop a data-driven approach to synthesising realistic head motions. 
