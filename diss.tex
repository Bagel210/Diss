...
Gestures are a huge part of communication \cite{kendon} and provide information that adds meaning to communication. In the recent years we have a seen a large increase in embodied conversational agents as a form of human - computer interface which replicate realistic human features to make the interaction seem as natural as possible and provide a rich user experience. These systems need to synthesise facial features and facial gestures that appear natural to humans. Systems that synthesise realistic natural head motions have typically used speech as base, however it can be difficult and expensive to record said speech. 

The goal of this project was to develop a system which is capable of synthesising realistic, life-like head motions using text as it's sole input so that speech is not needed so that systems can have avatars with realistic animations without having to spend time and money recording hours of speech.

The project is presented in several chapters. Chapter 2 outlines the background knowledge of this area of research as well as the techniques and tools used in the project. The related work is also contained in this chapter. Chapter 3 focuses on the design of the Text Driven Talking Heads system and the hypotheses about the relation between the text and head motions are presented.  Chapter 4 draws from the design explaining the implementation details of the system, and how the system was developed. The 5th chapter contains the evaluation of the system in which two methods of evaluation are presented and experiments are conducted in order to evaluate the system and the final chapter reviews the project and as well as discussing future improvements to the system. 
...
The task for the project was as follows: Using only text transcriptions of speech, synthesise life-like head motions that seem realistic and natural to humans. The project aimed to investigate the relationship between head motions and the information present in speech unique to the speaker. The difficulty of the task comes from the lack of unique information about the speech, as we are only using the transcribed text. To tackle this we will be using a Text-to-Speech system called Festival. 
...
Festival \cite{festival} is a full Text to Speech Synthesis software developed by the Centre of Speech and Technology Research at Edinburgh University. The software provides an entire framework for text analysis and speech synthesis. The system has many features including natural language processing tools, waveform synthesisers and scripting. The software is comprised of many modules which can be independently swapped out and improved and is fully customisable so that it can be fine tuned for specific purposes. 

Festival is able to be used for many different purposes where actual speech cannot be used like screen readers for the visually impaired, dictation, video game dialogue. In this project speech was not usable, and because of Festival's transparency, information regarding the synthesised speech was available meaning it could be used to help synthesise head motions.
...
Head Motion synthesis is the process of turning information we know about the environment like speech or text into motion trajectories. The goal of this project was to develop a system that was capable of synthesising life-like, realistic head motions when text is the only information that is available.
...
Below is an example of the information assigned to the input text of "Hello World". Festival has assigned the world Hello the pos tag 'UH' which is an interjection and assigned the word 'world' 'NN' for singular noun. Also in the example Festival has assigned break levels to the words. The interjection has been assigned a no break 'NB' and 'world' has been assigned a break, showing it is the end of the phrase.

\begin{lstlisting}
name Hello ; pos_index 16 ; pos uh ; phr_pos uh ; phrase_score -13.43 ; pbreak_index 1 ; pbreak NB ; 
name World ; pos_index 8 ; pos nn ; phr_pos n ; pbreak_index 0 ;  pbreak B ; blevel 3 ; 
\end{lstlisting}
...
As well as describing the intonation events symbolically Festival also assigns the fundamental frequency value to the text to each unit of speech called a phoneme in the text. A word is a combination of phonemes.

\begin{lstlisting}
113 id _151 ; f0 84.1413 ; pos 4.33792 ; 
\end{lstlisting}
...
Below is an example of the stress information that has been assigned to the inputted text 'Hello World' which has three syllables.

\begin{lstlisting}
id _6 ; name syl ; stress 0 ; 
id _9 ; name syl ; stress 1 ; 
id _12 ; name syl ; stress 1 ; 
\end{lstlisting}
...
The line of code (save\_waves\_during\_tts) meant that Festival saved all synthesised speech to wave files. An issue that raised from this was that it generated an audio file for each sentence, which was not suitable for the Text-Driven Talking Heads System. This problem was solved by implementing a function in the preparation stage called 'combine\_audio\_files' which created a new file that was the concatenation of all the sentences. This was a suitable solution as the produced output sounded fairly natural.

The Poser Script setMotion.py was taken from another Project that used Euler Angles to rotate a character's limbs, the only alterations made were to set the active body part to the head and to add the speech to the scene.
...
Volunteers were shown 5 different video clips and were asked to evaluate how natural they felt the head motions were. The videos shown were head motions trajectories mapped to a 3D avatar in Poser. 3 of the videos were synthesised from the Text Driven Talking Heads system and 2 videos were taken from real recordings. 

The three synthesised videos were from the Basic System, the Random system and the Rule Based system and were all generated using the same input text of 'there once lived a man and his wife who had long wished for a child but in vain'. The other two videos were the head motions taken from real recordings when the participants said the same sentence. The two participants used were called Desmond and Jane and the videos are labelled respectively. As there had no be no indication which video was which, the video were simply labelled as number 1 through 5 in the system. The key is presented below.
\\
\\
\begin{description}
...
\end{description}
 ...
 Participants were not told anything about the videos and so did not know if the videos were synthesised or real. This was to eliminate any potential bias that could be introduced by telling the participants some of the videos were real and some were not and made sure that the participants focused on the task.
...
%
%As the random system was deemed to be more natural than the actual recorded head motions it shows that the recorded head motions are not what the typical person would deem natural.
...
\appendix
\chapter{Head Motion Trajectories}

The following figures are graphical visualisations of the Head Motion trajectories that were used in the evaluation. In the following graphs the red line represents the value of the Roll Euler angle. Yaw is shown as a green line and the blue line represents pitch.

The synthesised head motions (1,2 and 3) were generated from the same text 'there once lived a man and his wife who had long wished for a child but in vain'. The 4th and 5th graphs are visual representations of the head motion trajectories from the data recordings for the participants Desmond and Jane which were used in the subjective evaluation process.

\begin{figure}
	\centering
	\caption{Basic System Head Motion Trajectories.}
	\includegraphics[width=0.6\textwidth]{/traj/sine_out.png}
	\centering
	\caption{Random System Head Motion Trajectories}
	\includegraphics[width=0.6\textwidth]{traj/random_out.png}
	\caption{Rule Based Head Motion Trajectories}
	\includegraphics[width=0.6\textwidth]{traj/rule_out.png}
\end{figure}
...
\begin{figure}
	...
	\includegraphics[width=0.6\textwidth]{traj/desmond_out.png}
	\caption{Head Motion Trajectories from participant 'Jane'}
	\includegraphics[width=0.6\textwidth]{traj/jane_out.png}
\end{figure}
